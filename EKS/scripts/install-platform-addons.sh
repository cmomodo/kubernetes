#!/usr/bin/env bash
# Bootstrap installer for shared Kubernetes add-ons.
#
# PREFERRED MODEL (GitOps): Argo CD manages platform Helm releases declaratively
# via gitops/applications/platform/*.yaml. This script is then only needed once
# to bootstrap Argo CD itself (or for disaster recovery / CI-controlled installs).
# Day-2 upgrades to cert-manager, external-dns, Traefik, Prometheus, and Argo CD
# should be done by bumping targetRevision / values in Git and letting Argo sync,
# not by re-running this script.
#
# COMPATIBILITY: The script remains fully functional for local and CI use.
...
#
# Controlled CI: .github/workflows/platform-addons.yaml runs this script with
# OIDC credentials on workflow_dispatch or when helm-values/ changes.
...
prometheus_chart_version="${PROMETHEUS_CHART_VERSION:-86.0.1}"
...
helm upgrade --install prometheus prometheus-community/kube-prometheus-stack \
  ...
  --values "$repo_root/helm-values/prometheus.yaml" \
  --atomic \
  --wait \
  --timeout 15m
